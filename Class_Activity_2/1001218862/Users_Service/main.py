...
def delete_user(user_id):
    ...
    try:
        # Llamar al servicio de tareas para eliminar las tareas asociadas
        response = requests.delete(f'http://localhost:5002/tasks/user/{user_id}')
        if response.status_code != 200:
            return jsonify({'error': 'Error al eliminar las tareas asociadas'}), 500
    except Exception as e:
        return jsonify({'error': f'Error de conexión al eliminar tareas: {str(e)}'}), 500

    db.session.delete(user)
    db.session.commit()
    return jsonify({'message': f'Usuario con ID {user_id} eliminado correctamente, junto con sus tareas'}), 200
